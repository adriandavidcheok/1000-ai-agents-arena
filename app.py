...
Do NOT invent anything. Output ONLY the BibTeX @article or @book entry."""
        try:
            resp = client.chat.completions.create(model=model, messages=[{"role": "system", "content": prompt}], temperature=0.7, **get_max_tokens_kw(model, 800))
...
Check if they look like real, existing academic references.
Flag any that seem imaginary or hallucinated.
Output only: "All references appear real" or list any suspicious ones."""
    try:
        resp = client.chat.completions.create(model=model, messages=[{"role": "system", "content": prompt}], temperature=0.7, **get_max_tokens_kw(model, 2000))
